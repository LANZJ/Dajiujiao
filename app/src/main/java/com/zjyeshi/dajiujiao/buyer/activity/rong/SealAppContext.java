...
import com.zjyeshi.dajiujiao.buyer.activity.rong.message.module.SealExtensionModule;
import com.zjyeshi.dajiujiao.buyer.activity.rong.server.broadcast.BroadcastManager;
import com.zjyeshi.dajiujiao.buyer.activity.rong.server.response.ContactNotificationMessageData;
import com.zjyeshi.dajiujiao.buyer.activity.rong.server.utils.NLog;
import com.zjyeshi.dajiujiao.buyer.activity.rong.server.utils.json.JsonMananger;
    ...
    public boolean onConversationClick(Context context, View view, UIConversation uiConversation) {
        ...
        if (messageContent instanceof ContactNotificationMessage) {
            ...
            if (contactNotificationMessage.getOperation().equals("AcceptResponse")) {
                ...
                context.startActivity(new Intent(context, MianFramActivity.class));
            }
            return true;
        }
        return false;
...
//                                    null));
    ...
    public UserInfo getUserInfo(final String s) {
      ...
      //  SealUserInfoManager.getInstance().getUserInfo(s);
        GetUserInfoTask.getUserTodo(mContext, s, new AsyncTaskSuccessCallback<UserData>() {
            @Override
            public void successCallback(Result<UserData> result) {
                if(result.getValue().getPic()==null)
                    return;
                Uri pic =  Uri.parse((String)result.getValue().getPic());
                UserInfo  userInfo = new UserInfo(s,result.getValue().getName(),pic);
                RongIM.getInstance().refreshUserInfoCache(userInfo);
                RongContext.getInstance().getUserInfoFromCache(s);
            }
        });
...
//            context.startActivity(intent);
            ...
            intent.putExtra(MEMBERID, userInfo.getUserId());
            context.startActivity(intent);
        }
        return true;
    }
    ...
    public void popAllActivity() {
        try {
            if (MianFramActivity.mViewPager != null) {
                MianFramActivity.mViewPager.setCurrentItem(0);
            }
