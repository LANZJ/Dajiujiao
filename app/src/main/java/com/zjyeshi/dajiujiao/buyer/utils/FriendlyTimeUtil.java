 ...
 */
public abstract class FriendlyTimeUtil {
	...
	public static boolean isSameDay(Date time1 , Date time2){
		...
		long ltd = time1.getTime() / DAY_INTER;
		long ctd = time2.getTime() / DAY_INTER;
		int days = (int) (ctd - ltd);
		if (days == 0){
			return true;
		}else{
	...
	public static boolean isSameDayByMillons(long time1 , long time2){
		long ltd = time1 / DAY_INTER;
		long ctd = time2 / DAY_INTER;
		int days = (int) (ctd - ltd);
		if (days == 0){
			return true;
		}else{
	 ...
	 * @return
	 */
	public static boolean isMoreThanFive(long time1 , long time2){
		long firstMin = time1/MINUTE_INTER;
		long secondMin = time2/MINUTE_INTER;

		int minutes = (int) (secondMin - firstMin);
		if (minutes >= 5){
			return true;
		}else{
			return false;
