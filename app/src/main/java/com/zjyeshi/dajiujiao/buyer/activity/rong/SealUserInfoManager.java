...
import com.zjyeshi.dajiujiao.buyer.activity.rong.db.BlackList;
import com.zjyeshi.dajiujiao.buyer.activity.rong.db.BlackListDao;
import com.zjyeshi.dajiujiao.buyer.activity.rong.db.DBManager;
import com.zjyeshi.dajiujiao.buyer.activity.rong.db.Friend;
import com.zjyeshi.dajiujiao.buyer.activity.rong.db.FriendDao;
import com.zjyeshi.dajiujiao.buyer.activity.rong.db.GroupMember;
import com.zjyeshi.dajiujiao.buyer.activity.rong.db.GroupMemberDao;
import com.zjyeshi.dajiujiao.buyer.activity.rong.db.Groups;
import com.zjyeshi.dajiujiao.buyer.activity.rong.db.GroupsDao;
import com.zjyeshi.dajiujiao.buyer.activity.rong.db.UserInfoBean;
import com.zjyeshi.dajiujiao.buyer.activity.rong.server.SealAction;
import com.zjyeshi.dajiujiao.buyer.activity.rong.server.network.async.AsyncTaskManager;
import com.zjyeshi.dajiujiao.buyer.activity.rong.server.network.async.OnDataListener;
import com.zjyeshi.dajiujiao.buyer.activity.rong.server.network.http.HttpException;
import com.zjyeshi.dajiujiao.buyer.activity.rong.server.pinyin.CharacterParser;
import com.zjyeshi.dajiujiao.buyer.activity.rong.server.response.GetBlackListResponse;
import com.zjyeshi.dajiujiao.buyer.activity.rong.server.response.GetGroupInfoResponse;
import com.zjyeshi.dajiujiao.buyer.activity.rong.server.response.GetGroupMemberResponse;
import com.zjyeshi.dajiujiao.buyer.activity.rong.server.response.GetGroupResponse;
import com.zjyeshi.dajiujiao.buyer.activity.rong.server.response.GetTokenResponse;
import com.zjyeshi.dajiujiao.buyer.activity.rong.server.response.UserRelationshipResponse;
import com.zjyeshi.dajiujiao.buyer.activity.rong.server.utils.NLog;
import com.zjyeshi.dajiujiao.buyer.activity.rong.server.utils.RongGenerate;
...
//                UserInfoEngine.getInstance(mContext).startEngine(userId);
                GetUserInfoTask.getUserTodo(mContext, LoginedUser.getLoginedUser().getId(), new AsyncTaskSuccessCallback<UserData>() {
                    @Override
                    public void successCallback(Result<UserData> result) {
                        Uri pic =  Uri.parse((String)result.getValue().getPic());
                        userInfo = new UserInfo(userId,result.getValue().getName(),pic);
                        RongIM.getInstance().refreshUserInfoCache(userInfo);
                    }
                });
//                GetUserInfoTask getUserInfoTask = new GetUserInfoTask(mContext);
...
//                    }
//                });
               // String ss="15820235157";
               // getUserInfoTask.execute(ss);
            }
        });
    }
    ...
    public void setUserInfoEngineListener() {
        UserInfoEngine.getInstance(mContext).setListener(new UserInfoEngine.UserInfoListener() {
            @Override
            public void onResult(final UserInfo info) {
...
//                }
                GetUserInfoTask.getUserTodo(mContext, "15820235157", new AsyncTaskSuccessCallback<UserData>() {
                    @Override
                    public void successCallback(Result<UserData> result) {
                        Uri pic =  Uri.parse((String)result.getValue().getPic());
                        userInfo = new UserInfo("1",result.getValue().getName(),pic);
                        RongIM.getInstance().refreshUserInfoCache(info);
                    }
                });
            }
