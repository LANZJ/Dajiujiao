...
import com.jopool.crow.imkit.utils.album.core.ImageLoader;
...
public class BucketImageActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
						...
						if (!ifMultiple) {
							...
							if (View.VISIBLE == bucketImageListItemView.imageViewSel
								...
								if (Temp.tempSelMap.size() >= limitCount
										&& -1 != limitCount) {
									// 提示不可选了
									CWToastUtil.displayTextShort("你最多只能选择"
											+ limitCount + "张照片");
								} else {
									bucketImageListItemView.imageViewSel
		...
		// 限制图片大小
		mScreenWidth = CWDisplayUtil.getDisplayMetrics(this).widthPixels;
		padding = (int) CWDisplayUtil.getPxByDp(this, 40);
