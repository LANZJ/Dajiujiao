package com.jopool.crow.imlib.utils.bitmap.core.cache;

import android.graphics.Bitmap;
import android.graphics.BitmapFactory;

import com.jopool.crow.imlib.utils.CWBitmapUtil;
...
import com.jopool.crow.imlib.utils.bitmap.BitmapGlobalConfig;
import com.jopool.crow.imlib.utils.bitmap.core.utils.BitmapCommonUtils;
import com.jopool.crow.imlib.utils.bitmap.core.utils.BitmapDecoder;
import com.jopool.crow.imlib.utils.io.IOUtils;
...
public class BitmapCache {
	...
	public BitmapCache initMemoryCache() {
		if (!globalConfig.isMemoryCacheEnabled()) {
			CWLogUtil.d("MemoryCache not enabled.");
			return null;
		}
	...
	public BitmapCache initDiskCache() {
		if (!globalConfig.isDiskCacheEnabled()) {
			CWLogUtil.d("DiskCache not enabled.");
			return null;
		}
	...
	private Bitmap addBitmapToMemoryCache(String cacheKey,
		...
		try {
			...
			CWLogUtil.e("读取图片内存溢出，原因：" + e.getMessage(), e);
		} catch (Exception e) {
			CWLogUtil.e(e.getMessage(), e);
		}
				...
				try {
					...
					if (snapshot != null) {
						...
						try {
							...
							CWLogUtil.e("解析图片内存溢出OOM错误，原因：" + e.getMessage(), e);
						}
						...
						/* 如果图片显示需要圆角处理，进行圆角处理 */
						if (config.getRoundPx() > 0) {
							bitmap = CWBitmapUtil.getRoundedCornerBitmap(bitmap,
									config.getRoundPx());
						}
					...
					CWLogUtil.e("读取磁盘缓存图片IO异常，原因：" + e.getMessage(), e);
				} finally {
					IOUtils.closeQuietly(snapshot);
				}
			}
		...
		synchronized (mDiskCacheLock) {
			if (mDiskLruCache != null && !mDiskLruCache.isClosed()) {
				try {
					...
					CWLogUtil.e("清理磁盘缓存异常，原因：" + e.getMessage(), e);
				}
				mDiskLruCache = null;
	...
	public void clearMemoryCache(String uri) {
		...
		if (CWValidator.isEmpty(keyList)) {
			return;
		}
		...
		synchronized (mDiskCacheLock) {
			if (mDiskLruCache != null && !mDiskLruCache.isClosed()) {
				try {
					...
					CWLogUtil.e("清理[" + uri + "]磁盘缓存IO异常，原因：" + e.getMessage(),
							e);
				}
			}
		}
		...
		synchronized (mDiskCacheLock) {
			if (mDiskLruCache != null) {
				try {
					...
					CWLogUtil.e("flush磁盘缓存IO异常，原因：" + e.getMessage(), e);
				}
			}
		}
	}
		...
		synchronized (mDiskCacheLock) {
			if (mDiskLruCache != null) {
				try {
					...
					CWLogUtil.e("关闭缓存IO异常，原因：" + e.getMessage(), e);
				}
			}
		}
	}
