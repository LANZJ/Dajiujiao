package com.jopool.crow.imlib.utils.bitmap.core.impl.local;
...
import com.jopool.crow.imlib.utils.asynctask.helper.CompatibleAsyncTask;
...
import com.jopool.crow.imlib.utils.bitmap.core.cache.LruMemoryCache;
import com.jopool.crow.imlib.utils.bitmap.core.impl.IBitmapLoader;
...
import com.jopool.crow.imlib.utils.bitmap.listeners.ClearCacheListener;
...
public class LocalBitmapLoaderImpl implements IBitmapLoader {
	...
	public void display(ImageView imageView, String filePath,
		...
		if (CWValidator.isEmpty(filePath)) {
		...
		if (defaultBitmapGlobalConfig.isMemoryCacheEnabled()) {
			...
			if (null != bitmap) {
				CWLogUtil.d("yes!!!cache is shot!!!");
				config.getDisplayImageListener().loadCompleted(imageView,
						bitmap, config);
				return;
			} else {
				CWLogUtil.d("no!!!cache is miss,i need get bitmap from disk!!!");
			}
		}
