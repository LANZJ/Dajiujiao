 ...
 * @author xuan
 */
public class UnBindChannelIdTask extends AbstractTask<String> {
	public UnBindChannelIdTask(Context context) {
		super(context);
	...
	protected Result<String> doHttpRequest(Object... params) {
		...
		try {
			CWHttpUtil.CWResponse response = CWHttpUtil.post(CWChat.getInstance().getConfig().getApiPrefix() + UrlConstants.URL_UNBIND_CHANNELID, paramMap);
