package com.jopool.crow.imlib.utils.media;
...
public class MediaRecorderModel {
	...
	public void startRecording(String voiceFileName) {
		...
		singleThreadPool.submit(new Runnable() {
			...
			public void run() {
				if (isStarted) {
					CWLogUtil.d("startRecord isStarted");
					return;
				}

				try {
					isStarted = true;
					CWLogUtil.d("startRecord");
					// 初始化MediaRecorder
	...
	public void stopRecording(final boolean isCancel) {
		singleThreadPool.submit(new Runnable() {
			...
			public void run() {
				if (!isStarted) {
					CWLogUtil.d("stopRecord !isStarted");
					return;
				}

				isStarted = false;
				try {
					CWLogUtil.d("stopRecord");
