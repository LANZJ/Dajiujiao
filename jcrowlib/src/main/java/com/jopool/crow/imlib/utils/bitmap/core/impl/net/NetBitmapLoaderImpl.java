package com.jopool.crow.imlib.utils.bitmap.core.impl.net;
...
import com.jopool.crow.imlib.utils.bitmap.core.cache.BitmapCacheManager;
...
public class NetBitmapLoaderImpl implements IBitmapLoader {
	...
	public void display(ImageView imageView, String uri,
		...
		if (null == imageView) {
			CWLogUtil.e("图片加载不处理，原因：图片显示控件imageView为空");
			return;
		}
		...
		if (null != bitmap) {
			if (DUBEG) {
				CWLogUtil.d("-----------------yes!!! memory cache shot!!!");
			}
		...
		protected Bitmap doInBackground(Object... params) {
			...
			if (!pauseTask && !this.isCancelled()
				...
				if (null != bitmap && DUBEG) {
					CWLogUtil.d("-----------------yes!!! disk cache shot!!!");
				}
			} else {
				if (DUBEG) {
					CWLogUtil.d("-----------------o god!!! i want disk cache,but you not need me anymore!!!");
				}
			}
			...
			if (null == bitmap && !pauseTask && !this.isCancelled()
					&& null != this.getTargetImageView()) {
				if (DUBEG) {
					CWLogUtil.d("-----------------no!!! cache not shot,i need download!!!");
				}
				...
				if (DUBEG) {
					CWLogUtil.d("-----------------o god!!! i want download,but you not need me anymore!!!");
				}
			}
